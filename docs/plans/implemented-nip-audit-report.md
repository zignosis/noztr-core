...
- The 2026-03-17 maintenance supplement records the second full implemented-surface audit batch:
  typed invalid-input fixes for `05`, `17`, `37`, `46`, `47`, `94`, and `99`, plus hostile
  example backfill for `03`, `17`, `37`, `42`, and `59`.
...
| 17 | complete | `HARNESS_COVERED BASELINE PASS` | `HARNESS_COVERED BASELINE PASS` | Overlong recipient/relay builder input now stays on typed `InvalidRecipientTag` / `InvalidRelayUrl` paths instead of depending on debug assertions; the bounded kind-14 / kind-15 helper surface otherwise remains accepted | none | none | The current helper remains protocol-kernel glue: bounded parse/build/unwrap reuse and relay-list extraction, with mailbox sync and delivery workflow still left to the SDK; hostile consumer-facing example coverage is now backfilled |
...
| 37 | complete | `LIB_UNSUPPORTED BASELINE PASS` | `LIB_UNSUPPORTED BASELINE PASS` | Strengthened the minimum draft-event validation floor so encrypted/decrypted draft JSON must now include at least `kind`, `tags`, and `content`, tightened private relay-list helpers to websocket relay URLs instead of generic HTTP-style URLs, and made overlong draft/identifier/relay builder input return typed invalid-input errors instead of depending on debug assertions | none | none | No dedicated helper exists in the active reference lanes; this remains a spec-first kernel helper, but the prior “any object” draft floor, generic-URL relay acceptance, and assertion-leaking overlong builder paths were all too broad for the module’s accepted contract |
...
| 42 | complete | `HARNESS_COVERED DEEP PASS` | `HARNESS_COVERED EDGE PASS` | Widened NIP-42 challenge bound from `64` to `255`; retained path-bound websocket origin matching, duplicate-tag rejection, and unbracketed IPv6 rejection, with hostile relay/challenge mismatch example coverage now backfilled | none | none | `rust-nostr` and `nostr-tools` both accept long challenges; current remaining strictness is judged trust-boundary-positive rather than ecosystem-hostile |
...
| 46 | complete | `HARNESS_COVERED BASELINE PASS` | `HARNESS_COVERED BASELINE PASS` | No Layer 1 change required; current message/URI/envelope/discovery surface remains compatible, deterministic `<nostrconnect>` template substitution stays accepted under the refined kernel boundary, and overlong public URI/query/relay/name/url input now returns typed errors instead of depending on debug assertions | none | none | The current helper stays on the protocol-data side of the boundary: parse/validate/build/discovery/template substitution in `noztr`, while relay/session/auth/orchestration remains SDK work |
...
| 59 | complete | `HARNESS_COVERED DEEP PASS` | `HARNESS_COVERED EDGE PASS` | No further Layer 1 change required; current staged wrap->seal->rumor validation and minimal one-recipient outbound build remain justified and compatible | none | none | Wrap-kind, signed-seal, unsigned-rumor, sender-continuity, outbound sender/rumor matching, and decrypt-failure boundaries all match the protocol intent; hostile consumer-facing example coverage is now backfilled |
...
| 94 | complete | `HARNESS_COVERED BASELINE PASS` | `LIB_UNSUPPORTED BASELINE PASS` | Accepted bounded kind-`1063` parse/build/validate with required `url` / lowercase MIME / `x`, exact supported-tag shapes, and repeated fallback support; overlong builder input and zero-width dimensions now stay on typed invalid-input paths instead of depending on debug assertions | none | none | Rust provides generic file-metadata support while `nostr-tools` remains a non-dedicated audit signal here; the current helper stays deterministic and metadata-only |
| 99 | complete | `LIB_UNSUPPORTED BASELINE PASS` | `LIB_UNSUPPORTED BASELINE PASS` | Tightened `d` so listing identifiers must be scheme-less URL-shaped values rather than generic non-empty UTF-8, and made overlong builder input return typed invalid-tag failures instead of depending on debug assertions | none | none | Neither active reference lane exposes a dedicated NIP-99 helper, so the stronger identifier rule and typed overlong-input handling are spec-first and keep addressable listing metadata deterministic without pulling commerce workflow into the kernel |
...
| 47 | complete | `HARNESS_COVERED BASELINE PASS` | `NOT_COVERED_IN_THIS_PASS applesauce lane unavailable locally` | Overlong direct token input now stays on typed `InvalidCapability` / `InvalidEncryptionTag` / `InvalidNotificationsTag` / `InvalidErrorObject` / `InvalidTransaction` paths, direct error/transaction token parsers no longer reuse unrelated error variants, and public connection URI/query/relay/lud16 validation no longer depends on debug assertions | none | none | Requested-loop supplement: the accepted split surface stays on deterministic NWC URI handling, event-envelope extraction, and typed decrypted JSON contracts; relay, encryption, and wallet workflow remain SDK-side |
...
- NIP-05: accept and canonicalize uppercase local-parts for lookup/output while keeping the exact
  NIP character set, bounded document extraction, and typed invalid-input handling for overlong
  address/domain/relay values.
...
- NIP-17: keep the bounded kind-14 / kind-15 parse/build and unwrap-reuse surface in the kernel,
  keep overlong builder input on typed `InvalidRecipientTag` / `InvalidRelayUrl` paths instead of
  debug assertions, and leave mailbox / delivery orchestration to the SDK.
...
- NIP-46: no Layer 1 change required; keep the current message/URI/envelope/discovery surface plus
  deterministic `<nostrconnect>` template substitution in the kernel, keep public URI/query/relay
  invalid input on typed errors instead of debug assertions, and leave signer session
...
- NIP-37: keep the current draft-wrap / private-relay helper surface, but keep overlong
  draft-json, identifier, and relay builder inputs on typed invalid-input paths instead of debug
  assertions.
- NIP-42: widen the challenge bound to `255` bytes, but keep path-bound websocket origin matching,
  ...
  behavior, and keep a hostile relay/challenge mismatch example in the public teaching surface.
...
- NIP-94: no further Layer 1 change required; keep the current bounded kind-`1063`
  parse/build/validate surface with required lowercase MIME, exact supported-tag shapes, and typed
  invalid-input handling for overlong builder values and zero-width dimensions.
- NIP-99: require `d` to be a scheme-less URL-shaped identifier instead of generic non-empty UTF-8,
  and keep overlong builder inputs on typed invalid-tag paths instead of debug assertions while
  keeping the current bounded metadata-only listing helper surface.
...
- NIP-47: keep the split kernel surface for deterministic NWC URI handling, bounded event-envelope
  extraction, and typed decrypted JSON contracts, keep direct token-parser failures on semantically
  correct typed errors instead of unrelated variants, and keep public URI/query/relay/lud16
  invalid input on typed errors rather than debug assertions.
...
- NIP-59: no further Layer 1 change required; keep the current staged unwrap and deterministic
  one-recipient outbound-build surface, and keep the hostile consumer-facing example set current.
